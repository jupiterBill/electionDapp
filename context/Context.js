...
const registerVoter = async() => {
    ...
    if (walletAddr)
  {
    ...
    router.push("/cast-vote")
      ...
      //   ""
      // );
      alert(`registration unsucessful : ${error.data.message} please come back to this page to try again`);
    }
}
...
const castVote = async (candidateIndex)=>{
  ...
  try{
    ...
    alert(`couldn't cast vote : ${error.data.message} please come back to this page to try again`);
  }
}
...
const getAdmin = async()=>{
  ...
  try{
    const contract =await  contractConnection();
     admin = await contract.getAdmin();
     console.log("transaction successful " + admin );
  }catch(error){
    console.log(error);
  }
  return admin;
  ...
  return (
    <ElectionContext.Provider value = {{ connectWallet,getVTEnd,getAdmin,transferOwnership,connectedWallet,voters,getCandidate,getCandidateLength,actualVoters,voters,walletAddr, fetchContract, registerVoter, castVote, candidates,getTotalAndWinner,isVotingEnd,newElection}}>
        { children }
